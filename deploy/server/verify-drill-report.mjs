...
const allowed = ['schemaVersion', 'executedAt', 'baselineRelease', 'invalidMcpRollback', 'apacheRejectedConfigRestore', 'interruptedSwitchRollback', 'publicSmokeRollback', 'finalHealth', 'failureStatusCodes'];
...
if (report.schemaVersion !== '1.2.0' || !/^[0-9a-f]{40}$/.test(report.baselineRelease ?? '')) throw new Error('The drill report version or baseline is invalid');
...
const expectedStatuses = { invalidMcp: 1, apacheConfig: 1, interruptedSwitch: 143, publicSmoke: 1 };
if (!report.failureStatusCodes || typeof report.failureStatusCodes !== 'object' || Array.isArray(report.failureStatusCodes) || Object.keys(report.failureStatusCodes).length !== Object.keys(expectedStatuses).length) throw new Error('The drill failure status evidence is invalid');
for (const [control, expectedStatus] of Object.entries(expectedStatuses)) if (report.failureStatusCodes[control] !== expectedStatus) throw new Error(`The drill failure status for ${control} is invalid`);
