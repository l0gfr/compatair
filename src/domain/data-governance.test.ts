...
describe('public catalog data governance', () => {
	it('publishes the exact explored, fixed and parametric grains without conflating them', () => {
		expect(createCatalogScope(compressors, tools)).toEqual({
			...
			tool_count: 761,
			explorable_combination_count: 181_879,
			fixed_flow_tool_count: 750,
			fixed_verdict_count: 179_250,
	...
	it('measures every field with explicit denominators and separate source roles', () => {
		...
		expect(distributorSku).toMatchObject({ eligible_count: 1000, populated_count: 2, explicitly_sourced_count: 2, primary_source_count: 0, independently_corroborated_count: 0 });
	});
