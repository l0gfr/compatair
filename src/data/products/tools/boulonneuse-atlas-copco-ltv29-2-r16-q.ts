// Source facts: atlas-copco-reviewed-2026-09-26.json.
const product = {
  "id": "atlas-copco-8431063124",
  "slug": "boulonneuse-atlas-copco-ltv29-2-r16-q",
  "categoryId": "boulonneuse",
  "category": "Boulonneuse pneumatique",
  "label": "Boulonneuse pneumatique Atlas Copco LTV29-2 R16-Q",
  "brand": "Atlas Copco",
  "model": "LTV29-2 R16-Q",
  "mpn": "8431063124",
  "variant": {
    "familyId": "atlas-copco-20532353419",
    "label": "LTV29-2 R16-Q",
    "distinguishingAttributes": {
      "Vitesse à vide": "850 tr/min",
      "Couple maximal de la plage": "16 Nm",
      "Couple minimal de la plage": "9 Nm",
      "Masse": "1,3 kg",
      "Longueur": "351 mm",
      "Dimension de sortie": "1/4\"",
      "Type de sortie": "Carré mâle",
      "Carré d’entraînement": "1/4 pouce",
      "Forme": "Renvoi d’angle",
      "Réversible": "Oui",
      "Arrêt automatique": "Oui",
      "Distance axe-bord de tête": "11 mm",
      "Hauteur de tête": "44 mm",
      "Consommation à vide": "600 L/min (10 l/s)",
      "Pression maximale admise": "7 bar",
      "Pression de référence des performances": "6,3 bar"
    }
  },
  "demandModel": "fixed-flow",
  "workingPressureBar": {
    "min": 6.3,
    "typical": 6.3,
    "max": 7
  },
  "airflowLpm": {
    "min": 600,
    "typical": 600,
    "max": 600
  },
  "usagePattern": "intermittent",
  "confidence": "A",
  "connectorSize": "Entrée 1/4 pouce (norme de filetage non précisée) ; flexible intérieur 10 mm",
  "recommendedHose": {
    "innerDiameterMm": 10
  },
  "image": {
    "src": "/images/products/atlas-copco-8431063124-technical.webp",
    "alt": "Atlas Copco LTV29-2 R16-Q, référence 8431063124 : 600 L/min à vide, référence 6,3 bar",
    "sourceUrl": "https://www.atlascopco.com/en-ca/itba/products/assembly-solutions/pneumatic-assembly-tools/ltv29-2-r16-q-sku8431063124",
    "sourceLabel": "Repères techniques CompatAir d’après Atlas Copco"
  },
  "editorial": {
    "overview": "Atlas Copco LTV29-2 R16-Q, référence 8431063124, demande 600 L/min à vide selon la fiche fabricant. Le dimensionnement utilise la pression de référence de 6,3 bar documentée dans le catalogue Industrial Tools and Solutions UK. Vitesse à vide : 850 tr/min. Couple maximal de la plage : 16 Nm.",
    "verifiedFacts": [
      "Vitesse à vide : 850 tr/min.",
      "Couple maximal de la plage : 16 Nm.",
      "Couple minimal de la plage : 9 Nm.",
      "Masse : 1,3 kg.",
      "Consommation à vide : 600 L/min, convertis depuis 10 l/s.",
      "Flexible recommandé : 10 mm de diamètre intérieur."
    ],
    "limitations": [
      "La consommation à vide reste celle du fabricant. Aucun cycle de travail supposé ne la réduit dans le scénario de référence.",
      "La pression maximale admise de 7 bar n’est pas la pression de référence du débit. Aucune consommation à 7 bar n’est extrapolée.",
      "La pression de référence provient du catalogue UK, page 3, recoupé avec la référence individuelle. Le document ne fournit pas de courbe débit-pression pour cette fiche. Une évolution de la notice du modèle doit entraîner une nouvelle vérification.",
      "Ces valeurs déclarées ne constituent pas un essai physique de CompatAir. La fiche internationale ne prouve ni un stock actuel en France ni le contenu de l’offre d’un vendeur.",
      "La longueur du flexible n’est pas spécifiée ici. Le diamètre seul ne permet pas de valider les pertes du réseau."
    ]
  },
  "specifications": [
    {
      "label": "Vitesse à vide",
      "value": "850 tr/min",
      "evidenceIds": [
        "atlas-copco-8431063124-fiche-fabricant"
      ]
    },
    {
      "label": "Couple maximal de la plage",
      "value": "16 Nm",
      "evidenceIds": [
        "atlas-copco-8431063124-fiche-fabricant"
      ]
    },
    {
      "label": "Couple minimal de la plage",
      "value": "9 Nm",
      "evidenceIds": [
        "atlas-copco-8431063124-fiche-fabricant"
      ]
    },
    {
      "label": "Masse",
      "value": "1,3 kg",
      "evidenceIds": [
        "atlas-copco-8431063124-fiche-fabricant"
      ]
    },
    {
      "label": "Longueur",
      "value": "351 mm",
      "evidenceIds": [
        "atlas-copco-8431063124-fiche-fabricant"
      ]
    },
    {
      "label": "Dimension de sortie",
      "value": "1/4\"",
      "evidenceIds": [
        "atlas-copco-8431063124-fiche-fabricant"
      ]
    },
    {
      "label": "Type de sortie",
      "value": "Carré mâle",
      "evidenceIds": [
        "atlas-copco-8431063124-fiche-fabricant"
      ]
    },
    {
      "label": "Carré d’entraînement",
      "value": "1/4 pouce",
      "evidenceIds": [
        "atlas-copco-8431063124-fiche-fabricant"
      ]
    },
    {
      "label": "Forme",
      "value": "Renvoi d’angle",
      "evidenceIds": [
        "atlas-copco-8431063124-fiche-fabricant"
      ]
    },
    {
      "label": "Réversible",
      "value": "Oui",
      "evidenceIds": [
        "atlas-copco-8431063124-fiche-fabricant"
      ]
    },
    {
      "label": "Arrêt automatique",
      "value": "Oui",
      "evidenceIds": [
        "atlas-copco-8431063124-fiche-fabricant"
      ]
    },
    {
      "label": "Distance axe-bord de tête",
      "value": "11 mm",
      "evidenceIds": [
        "atlas-copco-8431063124-fiche-fabricant"
      ]
    },
    {
      "label": "Hauteur de tête",
      "value": "44 mm",
      "evidenceIds": [
        "atlas-copco-8431063124-fiche-fabricant"
      ]
    },
    {
      "label": "Consommation à vide",
      "value": "600 L/min (10 l/s)",
      "evidenceIds": [
        "atlas-copco-8431063124-fiche-fabricant"
      ]
    },
    {
      "label": "Pression maximale admise",
      "value": "7 bar",
      "evidenceIds": [
        "atlas-copco-8431063124-fiche-fabricant"
      ]
    },
    {
      "label": "Pression de référence des performances",
      "value": "6,3 bar",
      "evidenceIds": [
        "atlas-copco-8431063124-catalogue-uk"
      ]
    }
  ],
  "evidence": [
    {
      "id": "atlas-copco-8431063124-fiche-fabricant",
      "sourceUrl": "https://www.atlascopco.com/en-ca/itba/products/assembly-solutions/pneumatic-assembly-tools/ltv29-2-r16-q-sku8431063124",
      "sourceLabel": "Atlas Copco, LTV29-2 R16-Q, 8431063124",
      "sourceType": "manufacturer",
      "retrievedAt": "2026-09-26",
      "confidence": "A",
      "notes": "Caractéristiques individuelles versionnées, valeurs sources en L/s et CFM, empreinte SHA-256 de la fiche consultée."
    },
    {
      "id": "atlas-copco-8431063124-catalogue-uk",
      "sourceUrl": "https://www.atlascopco.com/content/dam/atlas-copco/industrial-technique/general/documents/catalogs/Industrial%20Tools%20and%20Solutions_uk.pdf#page=34",
      "sourceLabel": "Atlas Copco, Industrial Tools and Solutions UK, page PDF 34 ; conditions page 3",
      "sourceType": "manufacturer",
      "retrievedAt": "2026-09-26",
      "confidence": "A",
      "notes": "Référence 8431063124 présente au tableau. Conditions générales : air détendu à la pression atmosphérique, pression de travail 6,3 bar sauf exception. Le millésime de cette édition n’est pas affirmé."
    }
  ],
  "fieldSources": {
    "mpn": [
      "atlas-copco-8431063124-fiche-fabricant",
      "atlas-copco-8431063124-catalogue-uk"
    ],
    "airflowLpm": [
      "atlas-copco-8431063124-fiche-fabricant"
    ],
    "workingPressureBar": [
      "atlas-copco-8431063124-catalogue-uk",
      "atlas-copco-8431063124-fiche-fabricant"
    ],
    "connectorSize": [
      "atlas-copco-8431063124-fiche-fabricant"
    ],
    "recommendedHose": [
      "atlas-copco-8431063124-fiche-fabricant"
    ]
  },
  "notes": [
    "Le verdict concerne l’alimentation en air. Les exigences de couple, d’accessoire et de sécurité restent celles de la notice de l’outil."
  ]
};
export default product;
