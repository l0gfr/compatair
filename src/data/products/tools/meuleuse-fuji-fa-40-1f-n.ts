const product = {
	"id": "fuji-fa-40-1f-n",
	"slug": "meuleuse-fuji-fa-40-1f-n",
	"categoryId": "meuleuse",
	"category": "Meuleuse pneumatique",
	"label": "Meuleuse pneumatique Fuji FA-40-1F N",
	"brand": "Fuji",
	"model": "FA-40-1F N",
	"mpn": "5412102870",
	"distributorSkus": [],
	"identifierAliases": [],
	"variant": {
		"familyId": "fuji-449451",
		"label": "FA-40-1F N",
		"distinguishingAttributes": {
			"Vitesse à vide": "13500 tr/min",
			"Puissance maximale de l’outil": "1000 W",
			"Filetage de sortie": "M8",
			"Diamètre du disque (mm)": "100",
			"Entrée d’air": "3/8 pouce NPT"
		}
	},
	"connectorSize": "Entrée 3/8 pouce NPT, flexible intérieur 10 mm sur 5 m",
	"usagePattern": "continuous",
	"recommendedHose": {
		"innerDiameterMm": 10,
		"maximumLengthMeters": 5
	},
	"confidence": "A",
	"image": {
		"src": "/images/products/fuji-fa-40-1f-n-technical.webp",
		"alt": "Repères techniques Fuji FA-40-1F N : 1 260 L/min en charge, pression de travail 6,3 bar",
		"sourceUrl": "https://www.fujitools.com/en/products/5412102870",
		"sourceLabel": "Repères techniques CompatAir d’après la fiche et la notice Fuji"
	},
	"editorial": {
		"overview": "Fuji FA-40-1F N, référence 5412102870, présente une consommation en charge de 21 L/s, soit 1 260 L/min. La notice liée à cette fiche indique une pression de travail de 6,3 bar. Vitesse à vide : 13500 tr/min. Puissance maximale de l’outil : 1000 W.",
		"verifiedFacts": [
			"Vitesse à vide : 13500 tr/min.",
			"Puissance maximale de l’outil : 1000 W.",
			"Filetage de sortie : M8.",
			"Diamètre du disque (mm) : 100.",
			"Entrée 3/8 pouce NPT ; flexible de 10 mm de diamètre intérieur sur 5 m."
		],
		"limitations": [
			"La consommation en charge provient de la fiche individuelle. La notice liée par le fabricant indique une pression de travail de 6,3 bar ; la fiche ne fournit pas de courbe de consommation selon la pression.",
			"Le calcul conserve le débit en charge publié, sans facteur arbitraire réduisant le besoin pour un usage intermittent.",
			"Le diamètre intérieur de flexible publié concerne une longueur de 5 m. Une installation plus longue nécessite de vérifier sa perte de pression.",
			"Le catalogue international distingue des raccords et équipements selon les marchés. La disponibilité en France et la conformité de la référence livrée restent à confirmer auprès du fournisseur."
		]
	},
	"specifications": [
		{
			"label": "Vitesse à vide",
			"value": "13500 tr/min",
			"evidenceIds": [
				"fuji-5412102870-official"
			]
		},
		{
			"label": "Puissance maximale de l’outil",
			"value": "1000 W",
			"evidenceIds": [
				"fuji-5412102870-official"
			]
		},
		{
			"label": "Filetage de sortie",
			"value": "M8",
			"evidenceIds": [
				"fuji-5412102870-official"
			]
		},
		{
			"label": "Diamètre du disque (mm)",
			"value": "100",
			"evidenceIds": [
				"fuji-5412102870-official"
			]
		},
		{
			"label": "Alésage de l’abrasif",
			"value": "15.8 mm",
			"evidenceIds": [
				"fuji-5412102870-official"
			]
		},
		{
			"label": "Type d’abrasif",
			"value": "Type 27",
			"evidenceIds": [
				"fuji-5412102870-official"
			]
		},
		{
			"label": "Distance du bord à l’axe",
			"value": "25 mm",
			"evidenceIds": [
				"fuji-5412102870-official"
			]
		},
		{
			"label": "Commande",
			"value": "Levier de sécurité",
			"evidenceIds": [
				"fuji-5412102870-official"
			]
		},
		{
			"label": "Longueur de l’outil",
			"value": "247 mm",
			"evidenceIds": [
				"fuji-5412102870-official"
			]
		},
		{
			"label": "Poids de l’outil",
			"value": "1.9 kg",
			"evidenceIds": [
				"fuji-5412102870-official"
			]
		}
	],
	"evidence": [
		{
			"id": "fuji-5412102870-official",
			"sourceUrl": "https://www.fujitools.com/en/products/5412102870",
			"sourceLabel": "Fuji, fiche officielle FA-40-1F N, réf. 5412102870",
			"sourceType": "manufacturer",
			"retrievedAt": "2026-09-25",
			"confidence": "A",
			"notes": "Consommation en charge publiée : 21 L/s × 60 = 1260 L/min. Valeurs brutes, unités et empreinte de la fiche versionnées."
		},
		{
			"id": "fuji-5412102870-pressure-notice",
			"sourceUrl": "https://www.photos-videos.fujitools.com/content/dam/pim/itba/fuji/technical-documents/update2025/fa-20-4-423/9502000609_05.pdf#page=2",
			"sourceLabel": "Fuji, notice 9502000609, révision 05, page 2",
			"sourceType": "manual",
			"sourceRole": "primary",
			"retrievedAt": "2026-09-25",
			"confidence": "A",
			"notes": "Notice explicitement liée à la fiche individuelle. Lecture visuelle : outil conçu pour 0,63 MPa (6,3 bar), à ne pas dépasser en fonctionnement."
		}
	],
	"fieldSources": {
		"mpn": [
			"fuji-5412102870-official"
		],
		"airflowLpm": [
			"fuji-5412102870-official"
		],
		"workingPressureBar": [
			"fuji-5412102870-pressure-notice"
		],
		"connectorSize": [
			"fuji-5412102870-official"
		],
		"recommendedHose": [
			"fuji-5412102870-official"
		]
	},
	"notes": [
		"Caractéristiques déclarées par le fabricant ; CompatAir n’a pas réalisé de mesure physique de cet outil.",
		"Le filetage PT et le filetage NPT ne sont pas considérés comme interchangeables."
	],
	"demandModel": "fixed-flow",
	"workingPressureBar": {
		"min": 6.3,
		"typical": 6.3,
		"max": 6.3
	},
	"airflowLpm": {
		"min": 1260,
		"typical": 1260,
		"max": 1260
	}
};

export default product;
