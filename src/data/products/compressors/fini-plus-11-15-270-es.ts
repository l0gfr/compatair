const product = {
	"id": "fini-plus-11-15-270-es",
	"slug": "fini-plus-11-15-270-es",
	"brand": "Fini",
	"model": "PLUS 11-15-270 ES",
	"mpn": "V91NN92FNM001",
	"tankLiters": 270,
	"maxPressureBar": 15,
	"fadCurve": [
		{
			"pressureBar": 15,
			"litersPerMinute": 980
		}
	],
	"oilType": "oil",
	"confidence": "A",
	"status": "unknown",
	"image": {
		"src": "/images/products/fini-plus-11-15-270-es.webp",
		"alt": "Repères techniques Fini PLUS 11-15-270 ES, référence V91NN92FNM001",
		"sourceUrl": "https://finicompressors.com/wp-content/uploads/Catalogo-Micro-Plus_Fini_IT_04-2024_9990394.pdf#page=17",
		"sourceLabel": "Carte technique CompatAir, valeurs déclarées par le fabricant ; pas une photographie du produit"
	},
	"editorial": {
		"overview": "Fini PLUS 11-15-270 ES, référence V91NN92FNM001 : cuve de 270 L, pression maximale publiée de 15 bar. Le point documenté le plus élevé en pression fournit 980 L/min à 15 bar. Sécheur intégré (version ES).",
		"verifiedFacts": [
			"Débit restitué publié : 980 L/min à 15 bar.",
			"Vis lubrifiée, vitesse fixe. Puissance moteur publiée : 11 kW.",
			"Masse nette publiée : 329 kg."
		],
		"limitations": [
			"Un seul point de débit restitué est documenté. Aucune mesure aux autres pressions n’est inventée.",
			"Le taux de marche continu n’est pas établi dans cette fiche. La disponibilité commerciale reste à confirmer.",
			"Le catalogue 04-2024 reste disponible sur le site fabricant. La disponibilité locale est à confirmer."
		]
	},
	"specifications": [
		{
			"label": "Conditions du débit",
			"value": "Débit restitué ISO 1217 à la pression de service indiquée ; note du tableau.",
			"evidenceIds": [
				"fini-v91nn92fnm001-20260926"
			]
		},
		{
			"label": "Équipement",
			"value": "Sécheur intégré (version ES).",
			"evidenceIds": [
				"fini-v91nn92fnm001-20260926"
			]
		},
		{
			"label": "Dimensions publiées",
			"value": "1560x680x1510 mm",
			"evidenceIds": [
				"fini-v91nn92fnm001-20260926"
			]
		}
	],
	"evidence": [
		{
			"id": "fini-v91nn92fnm001-20260926",
			"sourceUrl": "https://finicompressors.com/wp-content/uploads/Catalogo-Micro-Plus_Fini_IT_04-2024_9990394.pdf#page=17",
			"sourceLabel": "Fini, catalogue MICRO / PLUS, édition 04-2024, p. 17, réf. V91NN92FNM001",
			"sourceType": "manufacturer",
			"retrievedAt": "2026-09-26",
			"confidence": "A",
			"notes": "Débit restitué ISO 1217 à la pression de service indiquée ; note du tableau."
		},
		{
			"id": "fini-v91nn92fnm001-20260926-lubrification",
			"sourceUrl": "https://finicompressors.com/wp-content/uploads/Catalogo-Micro-Plus_Fini_IT_04-2024_9990394.pdf#page=1",
			"sourceLabel": "Fini, catalogue MICRO / PLUS, édition 04-2024, lubrification, p. 1",
			"sourceType": "manufacturer",
			"retrievedAt": "2026-09-26",
			"confidence": "A",
			"notes": "Source du mode de lubrification uniquement."
		}
	],
	"fieldSources": {
		"mpn": [
			"fini-v91nn92fnm001-20260926"
		],
		"tankLiters": [
			"fini-v91nn92fnm001-20260926"
		],
		"maxPressureBar": [
			"fini-v91nn92fnm001-20260926"
		],
		"fadCurve": [
			"fini-v91nn92fnm001-20260926"
		],
		"oilType": [
			"fini-v91nn92fnm001-20260926-lubrification"
		],
		"powerKw": [
			"fini-v91nn92fnm001-20260926"
		],
		"weightKg": [
			"fini-v91nn92fnm001-20260926"
		]
	},
	"notes": [
		"Caractéristiques déclarées par le fabricant. Les comparaisons dépendent des conditions de débit et de pression documentées."
	],
	"powerKw": 11,
	"weightKg": 329
};

export default product;
