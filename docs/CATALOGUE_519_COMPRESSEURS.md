...
Le rapport hebdomadaire privé et la commande de priorisation utilisent également ce lecteur progressif, au lieu de charger les 436 Mio de JSON en une seule chaîne. Un essai sur le catalogue complet avec une demande de test couvrant tous les outils traite 120 530 déficits sous un tas de 128 Mio, avec un pic mémoire de 191,5 Mio : la limite de service de 256 Mio reste inchangée. Le test de régression impose un fichier plus grand que le tas disponible, contrôle le classement et vérifie qu'un fichier tronqué ne remplace pas le dernier rapport valide.
