...
Le MCP conserve sa limite de 256 Mio. Son chargement compact dérive l’identifiant d’un couple depuis les identifiants compresseur et outil déjà partagés, au lieu de mémoriser une troisième chaîne pour chacun des 298 750 couples. Les identifiants personnalisés restent intacts. L’index partagé regroupe des références triées par outil pour chaque compresseur et utilise une recherche binaire ; il évite une seconde entrée de table de hachage par verdict. Le benchmark reconstruit l’intégralité du JSON publié à partir des recherches dans cet index et la compare octet pour octet, puis vérifie les réponses HTTP. Le moteur de calcul et les données de verdict ne changent pas.
