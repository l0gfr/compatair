...
(function () {
	...
	var LABELS = { continuous: 'Oui, en continu', intermittent: 'Oui, par intermittence', compatible: 'Oui, compatible', compatible_with_limits: 'Compatible avec limites', information: 'Information', incompatible: 'Non, incompatible', insufficient_data: 'Données insuffisantes' };
	var RESPONSE_SCHEMAS = { '1.0.0': true, '2.0.0': true };
	...
	function validResponse(data, origin) {
		if (!data || typeof data !== 'object' || !Object.prototype.hasOwnProperty.call(RESPONSE_SCHEMAS, data.schemaVersion)) return null;
	...
	function requiredFad(result) {
		if (typeof result.requiredFadLpm === 'number') return result.requiredFadLpm;
		return result.metrics && typeof result.metrics.required_fad_lpm === 'number' ? result.metrics.required_fad_lpm : null;
	}

	function displayedVerdict(result) {
		var key = result.engine_verdict && LABELS[result.engine_verdict] ? result.engine_verdict : result.verdict;
		return LABELS[key] ? { key: key, label: LABELS[key] } : null;
	}
		...
		function update(options) {
			...
			return fetch(endpoint, { method: 'GET', headers: { Accept: 'application/json' }, credentials: 'omit', referrerPolicy: 'no-referrer', signal: controller && controller.signal })
				...
				.then(function (payload) {
					...
					var data = parsed.data; var result = data.compatibility; var displayed = displayedVerdict(result); var requiredFadLpm = requiredFad(result);
					if (!displayed) throw new Error('Verdict API invalide');
					view.version.textContent = 'Moteur ' + String(data.calculationVersion || 'inconnu');
					view.verdict.textContent = displayed.label; view.verdict.className = 'verdict ' + displayed.key;
					view.detail.textContent = data.compressor.brand + ' ' + data.compressor.model + ' × ' + data.tool.label + (requiredFadLpm !== null ? ' · FAD recommandé ' + Math.round(requiredFadLpm) + ' L/min.' : '.');
