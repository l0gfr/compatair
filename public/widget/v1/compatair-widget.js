...
(function () {
	...
	function safeDetailsUrl(value, origin) {
		try {
			...
			return url.origin === origin && /^\/compatibilite\/[a-z0-9-]+--[a-z0-9-]+\/$/.test(url.pathname) && !url.search && !url.hash ? url.href : null;
		} catch (_) { return null; }
	}
	...
	function validResponse(data, origin) {
		...
		var detailsUrl = safeDetailsUrl(data.detailsUrl, origin);
		if (!detailsUrl) return null;
		return { data: data, detailsUrl: detailsUrl };
	}
		...
		function update(options) {
			...
			return fetch(endpoint, { method: 'GET', headers: { Accept: 'application/json' }, credentials: 'omit', referrerPolicy: 'no-referrer', signal: controller && controller.signal })
				...
				.then(function (payload) {
					...
					view.link.href = parsed.detailsUrl; view.link.hidden = false;
					return true;
				})
