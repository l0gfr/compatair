...
export function buildAcquisitionReport(aggregate) {
	...
	for (const bucket of aggregate.buckets) {
		...
		const row = rows.get(key) ?? { channel: bucket.channel, template: bucket.template, views: 0, decision_requests: 0, successes: 0, insufficient_data: 0, incompatible: 0, errors: 0, canonical_follows: 0, offer_outbounds: 0, conversions: 0, citation_acknowledged: 0 };
