...
const root = resolve('dist'); const errors = []; const htmlFiles = []; const artifactPaths = new Set();
async function walk(directory) { for (const name of await readdir(directory)) { const file = join(directory, name); const info = await lstat(file); const label = relative(root, file); if (info.isSymbolicLink()) errors.push(`${label}: lien symbolique interdit dans l’artifact`); else if (info.isDirectory()) await walk(file); else if (!info.isFile()) errors.push(`${label}: type de fichier spécial interdit dans l’artifact`); else { artifactPaths.add(`/${label}`); if (name.endsWith('.map')) errors.push(`${label}: source map publique interdite`); else if (name.endsWith('.html')) htmlFiles.push(file); } } }
await walk(root);
