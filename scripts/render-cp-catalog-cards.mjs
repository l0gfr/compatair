...
for (const record of records) {
	const product = createCpToolDraft(record);
	const columns = [
		{ x: 82, label: 'EN CHARGE', value: number(product.airflowLpm.typical), unit: 'L/min' },
		{ x: 460, label: 'PRESSION MAX.', value: number(product.workingPressureBar.max), unit: 'bar dynamiques' },
		{ x: 830, label: 'FLEXIBLE', value: number(product.recommendedHose.innerDiameterMm), unit: 'mm int. sur 5 m' },
	];
...
${columns.map(c => `<text x="${c.x}" y="374" font-size="22" font-weight="700" fill="#53665b">${c.label}</text><text x="${c.x}" y="474" font-size="76" font-weight="700">${xml(c.value)}</text><text x="${c.x}" y="524" font-size="27">${c.unit}</text>`).join('')}
